...
Owner keys skip all credit checks (see ``shared.auth.verify_api_key``) and
...
from shared.api_keys import create_owner_key, init_api_key_tables  # noqa: E402


def main() -> None:
    if len(sys.argv) < 2:
        print("Usage: python scripts/create_owner_key.py <email> [name]")
        raise SystemExit(1)
