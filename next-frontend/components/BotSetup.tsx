...
import type { Answer, Bot, SheetUpload } from "../lib/types";
...
export function SheetUploader({
  ...
  const send = async (file: File, source: "picker" | "drop") => {
    setBusy(true);
    setError(null);

    // The file's *shape*, never its name — a filename is customer data and
    // routinely contains a company name or a person's.
    const shape = {
      file_type: file.name.slice(file.name.lastIndexOf(".")).toLowerCase(),
      file_kb: Math.round(file.size / 1024),
      source,
      template_id: bot.template_id,
    };

    track(EVENTS.SHEET_UPLOAD_STARTED, shape);

    try {
      // Timed: indexing a sheet is the slowest thing a new customer waits
      // for, and it is the step they abandon. Knowing the p95 is the
      // difference between guessing and knowing whether that is why.
      const uploaded = await measure(
        "sheet_upload",
        () => api.uploadSheet(file),
        shape,
      );
      setResult(uploaded);
      onUploaded(uploaded);
      track(EVENTS.SHEET_UPLOAD_SUCCEEDED, {
        ...shape,
        documents_indexed: uploaded.documents_indexed,
        skipped_rows: uploaded.skipped_rows,
        warning_count: uploaded.warnings.length,
        category_count: uploaded.categories.length,
      });
    } catch (err) {
      setError((err as Error).message);
      track(EVENTS.SHEET_UPLOAD_FAILED, {
        ...shape,
...
export function AnsweringMode({
  ...
  const toggle = async () => {
    ...
    setError(null);
    try {
      const next = !bot.llm_enabled;
      onChange(await api.setAnsweringMode(next));
      // A settings change worth counting on its own: turning grounded
      // rewording on is the moment a customer trades verbatim answers for
      // fluent ones, and the adoption rate of that trade is the question
      // this feature exists to answer.
      track(EVENTS.BOT_UPDATED, {
        setting: "answering_mode",
        llm_enabled: next,
        template_id: bot.template_id,
      });
    } catch (err) {
...
export function BotTester({ bot }: { bot: Bot }) {
  ...
  const ask = async (text: string) => {
    ...
    setError(null);
    try {
      const result = await measure("bot_preview", () => api.preview(value));
      setAnswer(result);
      // The question itself is never sent — it is the customer's content and
      // often their end users'. What is worth knowing is the *outcome*: a
      // tester full of `decline` means the sheet does not cover what the
      // owner expected it to, which is the single best predictor of a bot
      // that gets abandoned before launch.
      track(EVENTS.BOT_TESTED, {
        mode: result.mode,
        confidence: Math.round(result.confidence * 100) / 100,
        matched: Boolean(result.matched_question),
