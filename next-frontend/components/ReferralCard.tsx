...
interface ReferralCardProps {
  /**
   * Read on the server alongside the rest of the dashboard. Without it this
   * card is the one part of the page that still shipped a spinner and filled
   * itself in a round trip later.
   */
  initialSummary?: ReferralSummary | null;
}

export function ReferralCard({ initialSummary = null }: ReferralCardProps) {
  ...
  const seeded = useRef(Boolean(initialSummary));
  useEffect(() => {
    // Already rendered from the server. `load` stays because sending an invite
    // calls it to refresh the list.
    if (seeded.current) {
