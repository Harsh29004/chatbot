...
export function Logo({ size = 20 }: { size?: number }) {
  return (
    <Link href="/" className="logo" aria-label="Nexora AI home">
  ...
  return (
    <div className="announce">
      <div className="announce-inner">
        ...
        <Link href={ANNOUNCE.href} className="announce-cta">
...
export function Nav() {
  ...
  // react-router's NavLink knew which link was current; next/link does not, so
  // the active path is read here and compared where the styling needs it.
            ...
            {customer ? (
              <>
                {/* The boxed style marks the page you're on, not a fixed
                    ...
                    react-router's NavLink computed this from a render prop;
                    next/link has no equivalent, so the current path is read
                    from usePathname and compared here. */}
                <Link
                  href="/assistant"
                  className={
                    pathname === "/assistant"
                ...
                <Link
                  href="/dashboard"
                  className={
                    pathname === "/dashboard" ? "btn btn-secondary btn-sm" : "nav-link"
                  }
                >
                  Dashboard
                </Link>
...
export function CodeBlock({
  ...
  const copy = async () => {
    try {
      ...
      // The snippet title says which block was taken — the install snippet,
      // the curl example, the config. Copying the install snippet is the
      // last thing someone does before the bot goes live, so it is the
      // closest thing this product has to an activation signal.
      track(EVENTS.WIDGET_SNIPPET_COPIED, {
        snippet: title.slice(0, 60),
        line_count: code.length,
      });
    } catch {
      setCopied(false);
