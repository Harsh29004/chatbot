...
import { api, PUBLIC_API_ORIGIN } from "../lib/api-client";
import type { ReferralSummary, BotTemplate, Pricing as PricingData, Bot, CreatedKey, Dashboard as DashboardData } from "../lib/types";
...
interface DashboardProps {
  /**
   * The dashboard payload and the account's bot, both read on the server from
   * the session cookie. The page arrives populated instead of showing a
   * spinner while two authenticated round trips complete.
   */
  initialDashboard?: DashboardData | null;
  initialBot?: Bot | null;
  initialTemplates?: BotTemplate[] | null;
  initialPricing?: PricingData | null;
  initialReferrals?: ReferralSummary | null;
}
...
export function Dashboard({
  ...
  const [bot, setBot] = useState<Bot | null>(initialBot);
  ...
  useEffect(() => {
    // Seeded from the server on first render; `load` still exists because
    // every mutation below calls it to refresh.
    if (initialDashboard) return;
    void load();
  }, [load, initialDashboard]);

  const keyCount = data?.keys.length;
  const seededBot = useRef(Boolean(initialBot));
  useEffect(() => {
    // Skip only the very first run when the server already supplied the bot.
    // Later runs are triggered by the key count changing, which is exactly
    // when the bot's availability can have changed too.
    if (seededBot.current) {
      seededBot.current = false;
      return;
    }
    void loadBot();
  }, [loadBot, keyCount]);
    ...
    <Page>
      <main className="dash">
        <div className="wrap">
          ...
          {/* Referrals ------------------------------------------------- */}
          <div className="mb-5">
            <ReferralCard initialSummary={initialReferrals} />
          </div>
