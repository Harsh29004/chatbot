...
export function SignIn() {
  ...
  const router = useRouter();
  // react-router carried "where were they heading" in navigation state, which
  // the App Router has no equivalent for. It travels in the query string
  // instead — the server-side guard puts it there before the redirect.
  const params = useSearchParams();
  ...
  // The server already redirects a signed-in visitor away from this route, so
  // this only fires when a sign-in succeeds while the page is still mounted.
  if (customer) {
    router.replace(from);
    return null;
  }
  ...
  return (
    <AuthShell
      ...
      footer={
        <>
          No account yet? <Link href="/signup" className="accent">Create one</Link>
        </>
      }
    >
...
export function SignUp() {
  ...
  return (
    <AuthShell
      ...
      footer={
        <>
          Already have an account? <Link href="/signin" className="accent">Sign in</Link>
        </>
      }
    >
