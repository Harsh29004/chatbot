...
import type { AssistantMessage, AssistantStatus, AssistantThread } from "../lib/types";
...
interface AssistantProps {
  /** Status and thread list, read on the server from the session cookie. */
  initialStatus?: AssistantStatus | null;
  initialThreads?: AssistantThread[] | null;
}

export function Assistant({ initialStatus = null, initialThreads = null }: AssistantProps) {
  ...
  const send = async () => {
    ...
    try {
      await api.sendAssistantMessage(
        ...
        text,
        (delta) => {
          // Time to first token: the number that decides whether a streaming
          // reply feels instant or broken.
