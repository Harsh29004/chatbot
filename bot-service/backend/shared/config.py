...
# This service lives in bot-service/, so PROJECT_ROOT resolves to bot-service/
# rather than the repository root where the shared .env sits. Both are tried,
# nearest first, so a bot-service/.env can override the shared one without
# needing a separate copy of every setting.
try:
...
# login produces. There is still exactly one thing that decides who someone
# is on this API: ``nexora_session``. A Firebase ID token is never accepted as
# authentication for anything except the one exchange endpoint.
#
# The service account is read from the environment rather than a file on disk,
# because a file has to be shipped into every container and mounted somewhere,
# and the thing being shipped is a private key.
#
# Two supported shapes, checked in this order:
#
#   1. FIREBASE_SERVICE_ACCOUNT_JSON — the whole downloaded JSON on one line.
#      Easiest for a secret manager that stores blobs.
#   2. FIREBASE_PROJECT_ID + FIREBASE_CLIENT_EMAIL + FIREBASE_PRIVATE_KEY —
#      the three fields that actually matter, as separate variables. Easiest
#      for a .env file and for reading at a glance which project is wired up.
#
# GOOGLE_APPLICATION_CREDENTIALS (a path) still works as a last resort, which
# is what Cloud Run and GKE inject on their own.
...
# is stored with literal "\n" and unescaped here. Without this the key parses
# as a single line and every token verification fails with an opaque error.
FIREBASE_PRIVATE_KEY: str = os.getenv("FIREBASE_PRIVATE_KEY", "").replace("\\n", "\n").strip()

# Falls back to the OAuth project so a deployment that only sets the web
# config still verifies tokens against the right project.
FIREBASE_CREDENTIALS_FILE: str = os.getenv("GOOGLE_APPLICATION_CREDENTIALS", "").strip()

FIREBASE_AUTH_ENABLED: bool = bool(
    FIREBASE_SERVICE_ACCOUNT_JSON
    or (FIREBASE_PROJECT_ID and FIREBASE_CLIENT_EMAIL and FIREBASE_PRIVATE_KEY)
    or FIREBASE_CREDENTIALS_FILE
)

# How far this server's clock may drift from Google's when checking an ID
# token's timestamps. Zero rejects a token issued one second "in the
# future" by our clock, which a VM a second behind hits on every sign-in.
# Signature, issuer and audience are still checked.
FIREBASE_CLOCK_SKEW_SECONDS: int = int(os.getenv("FIREBASE_CLOCK_SKEW_SECONDS", "30"))

# Refuse an ID token whose email Firebase has not confirmed. This is the same
# rule the OAuth path enforces, and for the same reason: matching an incoming
# identity to an existing account by email address is only sound if somebody
# proved they control that mailbox. Google-provider sign-ins are verified by
# Google; email+password sign-ups are not until the user clicks the link.
#
# Turning this off lets anyone who can type an address into your sign-up form
# take over the account that already owns it. Leave it on.
FIREBASE_REQUIRE_VERIFIED_EMAIL: bool = (
    os.getenv("FIREBASE_REQUIRE_VERIFIED_EMAIL", "true").lower() == "true"
)
...
# Unity only. The browser half of crash reporting is therefore ours: the page
# logs a GA4 ``exception`` event and POSTs the stack here, where it lands in
# the same MongoDB store as everything else.
#
# This endpoint is unauthenticated by necessity (a crash during sign-in still
# needs reporting), so it is rate limited per IP and every field is truncated.
TELEMETRY_ENABLED: bool = os.getenv("TELEMETRY_ENABLED", "true").lower() == "true"
TELEMETRY_MAX_ERRORS_PER_IP_PER_HOUR: int = int(
    os.getenv("TELEMETRY_MAX_ERRORS_PER_IP_PER_HOUR", "60")
)
