...
def template_adoption() -> dict[str, dict[str, int]]:
    ...
    Moved here from the admin query module when the bot pipeline went back to
    Python: it is a pure read of the ``bots`` collection, and that collection
    now has exactly one writer. The admin panel asks this service for it rather
    than counting rows it does not own.
    """
    ...
    for bot in coll(BOTS).find():
        ...
        bucket["bots"] += 1
        bucket["ready"] += 1 if bot.get("status") == STATUS_READY else 0
