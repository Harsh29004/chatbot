...
def get_customer_by_google_sub(google_sub: str) -> dict[str, Any] | None:
    ...
    Tried before email, because ``sub`` is the identifier that does not change.
    A user who changes their Gmail address is still the same account here.
    """
    return document(coll(CUSTOMERS).find_one({"google_sub": google_sub}))


def link_google_account(customer_id: Any, google_sub: str) -> dict[str, Any] | None:
    """
    Attach a Google identity to an existing account.

    Only ever called after Google has reported the address as verified — see
    ``google_oauth.py`` for why that check is the whole security of linking.
    ``auth_provider`` is left alone: an account that had a password keeps it,
    and both routes in.
    """
    oid = object_id(customer_id)
    if oid is None:
        return None
    coll(CUSTOMERS).update_one(
        {"_id": oid, "google_sub": None}, {"$set": {"google_sub": google_sub}}
    )
