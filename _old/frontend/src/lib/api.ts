...
export const api = {
   ...
   *
   * A 403 here means the email is not verified yet; the sign-in page turns
   * that into "check your inbox" plus a re-send button rather than a dead end.
   */
  firebaseExchange: (idToken: string, referralCode = "") =>
    post<Customer>("/auth/firebase", {
      id_token: idToken,
      referral_code: referralCode,
    }),
  ...
  authProviders: () =>
    request<{ password: boolean; firebase: boolean }>("/auth/providers"),
