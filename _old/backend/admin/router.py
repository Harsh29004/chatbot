...
@router.get("/crashes")
async def crashes(days: int = 7, limit: int = 50) -> dict[str, Any]:
    """
    ...
    ``affected_users`` counts distinct accounts rather than reports: one
    person reloading a broken page twenty times is one person with a problem,
    not twenty.
    """
    return {
        "groups": await asyncio.to_thread(
            telemetry_store.error_groups, _clamp_days(days), limit
        ),
        "window_days": _clamp_days(days),
    }


@router.get("/crashes/recent")
async def recent_crashes(limit: int = 100, release: str = "") -> dict[str, Any]:
    """
    The raw feed, newest first, with full stacks.

    What you open after ``/crashes`` tells you which group to care about.
    ``release`` narrows it to one build, which is the first thing worth
    checking when a crash rate jumps after a deploy.
    """
    return {
        "errors": await asyncio.to_thread(
            telemetry_store.recent_errors, limit, release.strip()
        )
    }
