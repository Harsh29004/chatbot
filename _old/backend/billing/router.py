...
from backend.billing import db, entitlements, firebase_auth, identity, referrals
...
async def auth_providers() -> dict[str, bool]:
    ...
    """
    return {"password": True, "firebase": firebase_auth.is_configured()}
...
# is indistinguishable from a password login. See backend/billing/
# firebase_auth.py for why the ID token is not simply trusted per-request.

@router.post("/auth/firebase", response_model=CustomerResponse)
async def firebase_exchange(
    body: FirebaseAuthRequest, request: Request, response: Response
) -> CustomerResponse:
    """
    Exchange a verified Firebase ID token for a session cookie.

    Handles both Firebase sign-in methods — Google and email+password — the
    same way, because by the time the token is verified the difference no
    longer matters: Firebase has said who this is, and what is left is finding
    or creating the account that belongs to them.
    """
    ...
    # Verification is a signature check plus a call to Firebase for the
    # revocation state, so it goes off the event loop.
    try:
        claims = await asyncio.to_thread(firebase_auth.verify_id_token, body.id_token)
        fb_identity = firebase_auth.identity_from_claims(claims)
    except firebase_auth.EmailNotVerifiedError as exc:
        # 403, not 401: the credential is good, the mailbox is the problem.
        # The page uses this to offer "resend verification email" instead of
    ...
    # A brand-new account here is a signup, and signups are rate limited per
    # IP for the same reason the password path limits them: distinct real
    # mailboxes, one person, one afternoon. Checked before creation, and only
    # when this really is a new account.
    is_new = (
        db.get_customer_by_firebase_uid(fb_identity["firebase_uid"]) is None
        and db.get_customer_for_mailbox(fb_identity["email"]) is None
        and (
            fb_identity["google_sub"] is None
            or db.get_customer_by_google_sub(fb_identity["google_sub"]) is None
        )
    )
    if is_new:
        try:
            identity.screen_signup_email(fb_identity["email"])
        except identity.EmailPolicyError as exc:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail=str(exc)
            ) from exc
    ...
    customer = _resolve_firebase_customer(
        fb_identity, body.referral_code or request.cookies.get(REFERRAL_COOKIE, "")
    )
    if customer is None:
        raise HTTPException(
            status_code=status.HTTP_409_CONFLICT,
...
def _resolve_firebase_customer(
    fb_identity: dict[str, Any], referral_code: str = ""
) -> dict[str, Any] | None:
    """
    ...
    3. **Known email** — an existing account, linked now. Only reachable
       because the unverified case was already refused: somebody proved they
       control that mailbox, so it is the same person. Their password, if they
       have one, keeps working.
    4. **Neither** — a new account, with no local password, trial started.
    """
    existing = db.get_customer_by_firebase_uid(fb_identity["firebase_uid"])
    if existing is not None:
        return existing

    google_sub = fb_identity.get("google_sub")
    if google_sub:
        by_google = db.get_customer_by_google_sub(google_sub)
        if by_google is not None:
            return db.link_firebase_account(
                by_google["id"],
                fb_identity["firebase_uid"],
                fb_identity["sign_in_provider"],
            )

    # Matched on the mailbox, so an account registered as "you@gmail.com" is
    # found even when the token reports "y.o.u@gmail.com".
    by_email = db.get_customer_for_mailbox(fb_identity["email"])
    ...
    try:
        customer = db.create_customer(
            ...
            firebase_uid=fb_identity["firebase_uid"],
            firebase_provider=fb_identity["sign_in_provider"],
        ...
        # Refusing is right: linking on a losing race is how one person ends
        # up inside another's account.
        return None

    _start_trial(customer, fb_identity["name"], referral_code)
    return customer
...
async def logout(
    ...
    if nexora_session:
        # Resolved before the session is revoked, because afterwards there is
        # no way back from the cookie to the account.
        ...
        # browser's word for it. Revoking the refresh tokens server-side means
        # a page that kept one cannot quietly mint a fresh ID token and open a
        # new session the user believes they closed. Best effort by design —
        # see firebase_auth.revoke_refresh_tokens.
        if customer and customer.get("firebase_uid"):
            await asyncio.to_thread(
                firebase_auth.revoke_refresh_tokens, customer["firebase_uid"]
            )
