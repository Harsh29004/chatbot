...
# Trust X-Forwarded-Proto/For from Caddy. Without this uvicorn only trusts
# 127.0.0.1, sees Caddy's plain-HTTP hop, and builds http:// URLs for an HTTPS
# site. Safe to trust any forwarder here: the port is published on loopback
# only, so nothing but Caddy on the compose network can reach it.
